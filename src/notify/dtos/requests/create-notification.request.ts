...
export class CreateNotificationRequest {
    @ApiModelProperty({
        required: true,
        description: 'Message code which will be sent to the recipient',
        example: 'COUPON_25'
    })
    @IsNotEmpty()
    ...
    @ApiModelProperty({
        required: true,
        enum: MethodsEnum,
        description: 'The method which will be used to notify the recipient'
    })
    @IsNotEmpty()
    ...
    @ApiModelProperty({
        ...
        isArray: true,
        type: Recipient,
        description: 'Recipient who would receive the notification'
    })
    @IsArray()
    @IsNotEmpty()
    @ValidateNested()
